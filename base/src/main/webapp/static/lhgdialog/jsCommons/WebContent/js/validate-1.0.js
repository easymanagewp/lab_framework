$(function() {
	 ...
	 * 校验失败后置方法
	 */
	var fail = function(message, $obj) {
		lhgdialog.setting.zIndex = 9999;
		$.dialog.tips(message,0.8,'alert.gif',function(){
	 ...
	 * 获取提示消息
	 */
	var getMessage = function($obj, validateType, validateTip) {
	...
	var validateTypes = {
		...
		suffix : function($obj) {
			...
			if (valSuffix == suffix) {
				...
				var sMessage = getMessage($obj, "suffix", "suffix").replace(
						"{suffix}", suffix);
				fail(sMessage);
				return false;
			}/*
				 * if(null!=uploadTest&&uploadTest.length>0){ var
				 * index=uploadTest.lastIndexOf("."); var
				 * newLoad=uploadTest.substring(index+1,uploadTest.length);
				 * if(newLoad=="rar"){ alert("不能上传rar压缩文件!"); return false; } }
				 */
			return true;
		},
		number : function($obj){
