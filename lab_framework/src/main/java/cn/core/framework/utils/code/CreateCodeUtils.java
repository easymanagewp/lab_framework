...
public class CreateCodeUtils {
	...
	public static String BASE_PROJECT_PATH = ApplicationUtils.getValue("config.project.baseDir").toString();
	public static String BASE_PROJECT_NAME = ApplicationUtils.getValue("config.project.basePackage").toString();
	public static String BASE_PROJECT_PACKAGE = BASE_PROJECT_NAME+".";
	public static String TEMPLATE_FILE_PACKAGE = "template";
	...
	public static void autoCreateCode(Class<? extends Object> entityCls,Boolean isReplace,ActionType at){
		EntityInfoBean eib = LoadEntityInfo.load(entityCls);
		FreemarkerUtils fts = new FreemarkerUtils(TEMPLATE_FILE_PACKAGE,FreemarkerUtils.LoadModel.CLASSPATH);
	...
	private static void createVo(Boolean isReplace, EntityInfoBean eib,
		...
		File voFileDir = new File(baseFile,eib.getVoPackageName().replace(".", File.separator));
		if(!voFileDir.exists()){
			voFileDir.mkdirs();
		}
		File daoFile = new File(voFileDir,eib.getSimpleEntityName()+"Vo.java");
	...
	private static void createDAOImpl(Boolean isReplace, EntityInfoBean eib,
			FreemarkerUtils fts) {
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getDaoImplPackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
			daoFileDir.mkdirs();
		}
		File daoFile = new File(daoFileDir,eib.getSimpleEntityName()+"DaoImpl.java");
		if(daoFile.exists() && isReplace){
			daoFile.delete();
		}
	...
	private static void createController(Boolean isReplace, EntityInfoBean eib,
			FreemarkerUtils fts) {
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getControllerPackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
			daoFileDir.mkdirs();
		}
		File xmlFile = new File(daoFileDir,eib.getSimpleEntityName()+"Action.java");
	...
	private static void createController4Jsp(Boolean isReplace, EntityInfoBean eib,
			FreemarkerUtils fts) {
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getControllerPackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
	...
	private static void createServiceImpl(Boolean isReplace,
		...
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getServiceImplPackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
			daoFileDir.mkdirs();
		}
		File xmlFile = new File(daoFileDir,eib.getSimpleEntityName()+"ServiceImpl.java");
		if(xmlFile.exists() && isReplace){
			xmlFile.delete();
		}
	...
	private static void createIService(Boolean isReplace, EntityInfoBean eib,
			FreemarkerUtils fts) {
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getServiceInterfacePackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
			daoFileDir.mkdirs();
		}
		File xmlFile = new File(daoFileDir,"I"+eib.getSimpleEntityName()+"Service.java");
		if(xmlFile.exists() && isReplace){
			xmlFile.delete();
		}
	...
	private static void createDAO(Boolean isReplace, EntityInfoBean eib,
		...
		File baseFile = new File(BASE_PROJECT_PATH);
		File daoFileDir = new File(baseFile,eib.getDaoInterfacePackageName().replace(".", File.separator));
		if(!daoFileDir.exists()){
			daoFileDir.mkdirs();
		}
		File daoFile = new File(daoFileDir,"I"+eib.getSimpleEntityName()+"Dao.java");
		if(daoFile.exists() && isReplace){
			daoFile.delete();
		}
