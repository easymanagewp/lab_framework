...
public class FreemarkerUtils {
	...
	private TemplateLoader getClassPathTeamplateLoader(final String templateFileDir) {
		return new URLTemplateLoader() {
			@Override
			protected URL getURL(String name) {
				
				String url = new StringBuffer(templateFileDir.replace(".", "/")).append("/").append(name).toString();
				URL _url = FreemarkerUtils.class.getClassLoader().getResource(url);
				if(_url == null){
					name = StringUtils.replace(name, "_"+cfg.getLocale().getLanguage()+"_"+cfg.getLocale().getCountry(), "");
					url = new StringBuffer(templateFileDir.replace(".", "/")).append("/").append(name).toString();
					 _url = FreemarkerUtils.class.getClassLoader().getResource(url);
				}
				return _url;
			}
		};
	...
	public void writer(String templatePath,String targetPath,Object data){
		try {
			...
			template.process(data, new OutputStreamWriter(new FileOutputStream(targetPath)));
		} catch (IOException e) {
			log.error(e.getMessage(), e);
		} catch (TemplateException e) {
			log.error(e.getMessage(),e);
