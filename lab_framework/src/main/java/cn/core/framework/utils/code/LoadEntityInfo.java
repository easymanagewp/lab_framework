...
public class LoadEntityInfo {
	public static EntityInfoBean load(Class<? extends Object> entityCls) {
		...
		entityInfoBean.setDaoInterfacePackageName(entityPackageName.replace("po","dao"));
		entityInfoBean.setDaoImplPackageName(entityInfoBean.getDaoInterfacePackageName()+".impl");
		entityInfoBean.setServiceInterfacePackageName(entityPackageName.replace("po","service"));
		entityInfoBean.setServiceImplPackageName(entityInfoBean.getServiceInterfacePackageName()+".impl");
		entityInfoBean.setControllerPackageName(entityPackageName.replace("po","action"));
		entityInfoBean.setEntityName(entityCls.getName());
		entityInfoBean.setVoPackageName(entityCls.getPackage().getName().replace("po", "vo"));
		...
		entityInfoBean.setEntityLoverCaseName(simpleEntityName.replaceFirst(start, start.toLowerCase()));
