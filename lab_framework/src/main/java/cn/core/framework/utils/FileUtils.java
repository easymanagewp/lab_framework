...
public class FileUtils extends org.apache.commons.io.FileUtils{
		...
		if(ArrayUtils.isEmpty(fileArray)){
			...
			for(File subFile : fileArray){
				if(subFile.isFile()){
					...
					List<File> subFileList = FileUtils.getFiles4Directory(subFile.getPath());
	...
	/**
	 * 获取文件路径 根据包名称
	 * @param packageName 依据的包的名称
	 * @return 文件的路径
	 */
	public static String getPath4PackageName(String packageName){
		return FileUtils.class.getClassLoader().getResource(packageName.replace(".", "/")).getPath();
	}
